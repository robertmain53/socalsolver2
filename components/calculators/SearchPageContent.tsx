    ...
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-gray-800">
          {t.search.resultsTitle[lang]}: <span className="text-indigo-600">"{query}"</span>
        </h1>
        <p className="mt-2 text-gray-600">
          {searchResults.length} {searchResults.length === 1 ? t.search.resultFound[lang] : t.search.resultsFound[lang]}
        </p>
      </div>
      ...
      {searchResults.length > 0 ? (
          ...
          {searchResults.map((calc) => (
            ...
            >
              ...
              <p className="text-sm text-gray-500 mt-1">
                {(t.category as any)[calc.category]?.[lang] || calc.category}
              </p>
        ...
        <div className="text-center py-16 px-6 bg-white rounded-xl shadow-lg">
          ...
          <h3 className="mt-4 text-lg font-semibold text-gray-800">{t.search.noResultsTitle[lang]}</h3>
          <p className="mt-2 text-sm text-gray-500">{t.search.noResultsText[lang]}</p>
        </div>
      )}
    </div>
  );
