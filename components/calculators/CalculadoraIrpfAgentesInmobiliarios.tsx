...
const calculatorData = {
  ...
  title: "Calculadora de IRPF para agentes inmobiliarios autónomos",
  lang: "es",
  tags: "calculadora irpf agente inmobiliario, impuestos inmobiliaria, modelo 130, iva inmobiliaria, gastos deducibles agente inmobiliario, autonomo inmobiliario, fiscalidad API",
  inputs: [
    { id: "ingresosTrimestrales", label: "Comisiones trimestrales (sin IVA)", type: "number" as const, unit: "€", min: 0, step: 500, tooltip: "Introduce el total de tus comisiones por ventas o alquileres facturadas en el trimestre." },
    { id: "gastosDeducibles", label: "Gastos deducibles del trimestre", type: "number" as const, unit: "€", min: 0, step: 100, tooltip: "Suma todos los gastos de tu actividad: portales inmobiliarios, gasolina, CRM, gestoría, marketing, etc." },
    { id: "porcentajeRetencion", label: "Tipo de retención IRPF (si aplica)", type: "select" as const, options: [{ value: 15, label: "15% (General)" }, { value: 7, label: "7% (Nuevos autónomos)" }], tooltip: "Aplica retención solo si facturas a una agencia inmobiliaria. Si facturas al cliente final (vendedor/comprador particular), no hay retención." },
    { id: "ingresosConRetencion", label: "Comisiones con retención", type: "number" as const, unit: "€", min: 0, step: 500, tooltip: "Introduce aquí solo las comisiones que has cobrado de una agencia que te ha practicado retención. Si facturas directo al cliente, es 0€." }
  ],
  outputs: [
  ...
  content: `### Introducción: Planifica tus Comisiones con Inteligencia

(Contenido omitido aquí por brevedad; es el mismo que me enviaste)
`,
  seoSchema: {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    "mainEntity": [
      { "@type": "Question", "name": "¿Puedo deducir los gastos de mi coche y la gasolina?", "acceptedAnswer": { "@type": "Answer", "text": "Sí, pero con matices importantes..." } },
      { "@type": "Question", "name": "Acabo de recibir una comisión de 10.000€. ¿De verdad tengo que pagar un 20% a Hacienda este trimestre?", "acceptedAnswer": { "@type": "Answer", "text": "Sí, el cálculo del Modelo 130 se basa en el beneficio..." } },
      { "@type": "Question", "name": "La cuota mensual que pago a Idealista, ¿es un gasto deducible?", "acceptedAnswer": { "@type": "Answer", "text": "Sí, al 100%..." } }
    ]
  }
} as const;
...
const formatCurrency = (value: number) =>
  new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR' }).format(toNum(value));

// --- Importazione Dinamica del Grafico (SSR-safe) ---
      ...
      <mod.BarChart data={data} layout="vertical" barSize={60}>
        ...
        <mod.Tooltip
          formatter={(value: number, name: string) => [
            `${formatCurrency(value)}${totalIncome > 0 ? ` (${((value / totalIncome) * 100).toFixed(1)}%)` : ''}`,
            name
          ]}
        />
...
const CalculadoraIrpfAgentesInmobiliarios: React.FC = () => {
  ...
  const initialStates = {
    ingresosTrimestrales: 12000,
    gastosDeducibles: 2800,
    porcentajeRetencion: 15,
    ingresosConRetencion: 6000
  };
  ...
  const {
    ...
    // Impuestos totales adelantados en el trimestre
    const impuestosTotales = Math.max(0, resultadoModelo130) + retencionesPracticadas;

    // Beneficio neto final tras gastos e impuestos (retenciones + pago 130 si procede)
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 p-4 md:p-6 bg-gray-50 font-sans">
        <main className="lg:col-span-2">
          <div className="bg-white rounded-lg shadow-md" ref={calculatorRef}>
            <div className="p-6">
              ...
              <p className="text-gray-600 mb-4">Calcula tu pago trimestral de IRPF (Modelo 130) y gestiona tus comisiones eficazmente.</p>
              <div className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-md p-3 mb-6">
                <strong>Planifica tu Tesorería:</strong> Dado que tus ingresos pueden ser irregulares, es vital apartar un ~25% de cada comisión para cubrir impuestos y la cuota de autónomos.
              </div>
                  ...
                  <div key={input.id}>
                    ...
                    {input.id === 'ingresosConRetencion' && (
                      <p className="text-xs mt-1">
                        {(() => {
                          const ing = toNum(states.ingresosTrimestrales);
                          const con = toNum(states.ingresosConRetencion);
                          if (con > ing) return <span className="text-red-600">No puede superar los ingresos totales</span>;
                          return null;
                        })()}
                      </p>
                    )}
              ...
              <div className="mt-8">
                <h3 className="text-lg font-semibold text-gray-700 mb-2">Desglose Visual de tus Comisiones</h3>
        ...
        <aside className="lg:col-span-1 space-y-6">
          ...
          <section className="border rounded-lg p-4 bg-white shadow-md">
            <h2 className="font-semibold mb-3 text-gray-800">Guía Fiscal para Agentes Inmobiliarios</h2>
          ...
          <section className="border rounded-lg p-4 bg-white shadow-md">
            ...
            <ul className="prose prose-sm max-w-none text-gray-700 list-disc pl-5 space-y-2">
              ...
              <li><a href="https://sede.agenciatributaria.gob.es/Sede/ayuda/manuales-videos-folletos/manuales-practicos/iva-2023/capitulo-9-deducciones/gastos-deducibles-reglas-comunes/cuotas-soportadas-bienes-servicios-uso-privado.html" target="_blank" rel="noopener noreferrer nofollow" className="text-indigo-600 hover:underline">Agencia Tributaria: Deducción de gastos de uso mixto</a></li>
            </ul>
          </section>
        </aside>
      </div>
...
export default CalculadoraIrpfAgentesInmobiliarios;
