...
const INITIAL_BACKOFF_MS = 30000; // Ridotto da 60s a 30s
...
class Logger {
    ...
    showProgress(current, total, item = '') {
        ...
        console.log(`\x1b[35m📊 API Stats\x1b[0m: Calls: ${this.stats.apiCalls} | Rate Limits: ${this.stats.rateLimitHits}`);
    }
...
console.log('\x1b[35m');
console.log('╔══════════════════════════════════════════════════╗');
console.log('║        🤖 SoCalSolver Professional v2.1         ║');
console.log('║       Script Automazione con Rate Limiting      ║');
console.log('╚══════════════════════════════════════════════════╝');
console.log('\x1b[0m');

await logger.info('INIT', 'Avvio sistema di automazione con rate limiting migliorato...');
...
// ========================================
// ENHANCED RATE LIMITED API CALLS
// ========================================
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
            ...
            if (response.ok) {
                ...
                await logger.debug('AI', `Contenuto ricevuto per ${requestType}`, { 
            ...
            if (response.status === 429) {
                ...
                await logger.warning('AI', `Rate limit Gemini per ${requestType}, pausa ${delay / 1000}s...`);
...
async function main() {
    try {
        ...
        await logger.info('PROGRESS', 'Inizio generazione batch con rate limiting', {
            totale: calculators.length,
            startIndex,
            rimanenti: remainingCount
        });

        console.log(`\n🚀 AVVIO GENERAZIONE BATCH CON RATE LIMITING`);
        ...
        console.log(`⏱️  Tempo stimato: ~${Math.round(remainingCount * 3)}min (180s per calcolatore con pause)`);
        console.log(`🤖 Rate limiting: 45s tra chiamate API, 2min tra calcolatori`);
        console.log('');

        // Loop di generazione con enhanced rate limiting
        for (let index = startIndex; index < calculators.length; index++) {
            ...
            // Mostra statistiche intermedie ogni 3 elementi
            if (current % 3 === 0) {
                logger.showStats();
            }

            // ===== PAUSA TRA CALCOLATORI COMPLETI =====
