...
async function getCalculators() {
    const calculatorsPath = path.join(process.cwd(), 'content', 'en', 'professional-and-specialized');
    try {
...
export default async function CategoryPage() {
  ...
  const categoryName = "Professional & Specialized";
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {calculators.map((calc) => (
            <Link 
              key={calc.slug} 
              href={`/en/professional-and-specialized/${calc.slug}`} 
