...
async function getCalculators() {
    const calculatorsPath = path.join(process.cwd(), 'content', 'es', 'educacion-y-universidad');
    try {
...
export default async function CategoryPage() {
  ...
  const categoryName = "Educación y Universidad";
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {calculators.map((calc) => (
            <Link 
              key={calc.slug} 
              href={`/es/educacion-y-universidad/${calc.slug}`} 
