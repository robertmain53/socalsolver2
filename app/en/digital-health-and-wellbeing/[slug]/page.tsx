...
const CATEGORY = 'digital-health-and-wellbeing';
const LANG = 'en';
...
async function getContent(slug: string) {
  try {
    const contentPath = path.join(process.cwd(), 'content', 'en', 'digital-health-and-wellbeing', `${slug}.md`);
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const crumbs = [
      { name: "Home", path: "/en" },
      { name: "Digital Health & Wellbeing", path: "/en/digital-health-and-wellbeing" },
      { name: calculatorName }
  ];
