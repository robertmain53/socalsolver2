...
async function getCalculators() {
    const calculatorsPath = path.join(process.cwd(), 'content', 'en', 'digital-health-and-wellbeing');
    try {
...
export default async function CategoryPage() {
  ...
  const categoryName = "Digital Health & Wellbeing";
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {calculators.map((calc) => (
            <Link 
              key={calc.slug} 
              href={`/en/digital-health-and-wellbeing/${calc.slug}`} 
