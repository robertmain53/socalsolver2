...
const CATEGORY = 'real-estate-and-housing';
const LANG = 'en';
...
async function getContent(slug: string) {
  try {
    const contentPath = path.join(process.cwd(), 'content', 'en', 'real-estate-and-housing', `${slug}.md`);
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const crumbs = [
      { name: "Home", path: "/en" },
      { name: "Real Estate & Housing", path: "/en/real-estate-and-housing" },
      { name: calculatorName }
  ];
