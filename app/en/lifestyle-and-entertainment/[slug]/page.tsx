...
const CATEGORY = 'lifestyle-and-entertainment';
const LANG = 'en';
...
async function getContent(slug: string) {
  try {
    const contentPath = path.join(process.cwd(), 'content', 'en', 'lifestyle-and-entertainment', `${slug}.md`);
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const crumbs = [
      { name: "Home", path: "/en" },
      { name: "Lifestyle & Entertainment", path: "/en/lifestyle-and-entertainment" },
      { name: calculatorName }
  ];
