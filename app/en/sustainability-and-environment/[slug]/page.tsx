...
const CATEGORY = 'sustainability-and-environment';
const LANG = 'en';
...
async function getContent(slug: string) {
  try {
    const contentPath = path.join(process.cwd(), 'content', 'en', 'sustainability-and-environment', `${slug}.md`);
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const crumbs = [
      { name: "Home", path: "/en" },
      { name: "Sustainability & Environment", path: "/en/sustainability-and-environment" },
      { name: calculatorName }
  ];
