...
async function getCalculators() {
    const calculatorsPath = path.join(process.cwd(), 'content', 'en', 'sustainability-and-environment');
    try {
...
export default async function CategoryPage() {
  ...
  const categoryName = "Sustainability & Environment";
  const crumbs = [{ name: "Home", path: "/en" }, { name: categoryName }];
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {calculators.map((calc) => (
            <Link 
              key={calc.slug} 
              href={`/en/sustainability-and-environment/${calc.slug}`} 
