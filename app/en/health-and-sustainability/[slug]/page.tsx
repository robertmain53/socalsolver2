...
const CATEGORY = 'health-and-sustainability';
const LANG = 'en';
...
async function getContent(slug: string) {
  try {
    const contentPath = path.join(process.cwd(), 'content', 'en', 'health-and-sustainability', `${slug}.md`);
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const crumbs = [
      { name: "Home", path: "/en" },
      { name: "Health & Sustainability", path: "/en/health-and-sustainability" },
      { name: calculatorName }
  ];
