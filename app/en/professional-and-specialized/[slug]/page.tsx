...
const CATEGORY = 'professional-and-specialized';
const LANG = 'en';
...
async function getContent(slug: string) {
  try {
    const contentPath = path.join(process.cwd(), 'content', 'en', 'professional-and-specialized', `${slug}.md`);
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const crumbs = [
      { name: "Home", path: "/en" },
      { name: "Professional & Specialized", path: "/en/professional-and-specialized" },
      { name: calculatorName }
  ];
