...
const CATEGORY = 'business-and-marketing';
const LANG = 'en';
...
async function getContent(slug: string) {
  try {
    const contentPath = path.join(process.cwd(), 'content', 'en', 'business-and-marketing', `${slug}.md`);
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const crumbs = [
      { name: "Home", path: "/en" },
      { name: "Business & Marketing", path: "/en/business-and-marketing" },
      { name: calculatorName }
  ];
