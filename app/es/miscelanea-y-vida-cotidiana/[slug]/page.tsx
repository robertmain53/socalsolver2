...
async function getContent(slug: string) {
  try {
    const contentPath = path.join(process.cwd(), 'content', 'es', 'miscelanea-y-vida-cotidiana', `${slug}.md`);
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const crumbs = [
      { name: "Home", path: "/es" },
      { name: "Miscelánea y vida cotidiana", path: "/es/miscelanea-y-vida-cotidiana" },
      { name: calculatorName }
  ];
