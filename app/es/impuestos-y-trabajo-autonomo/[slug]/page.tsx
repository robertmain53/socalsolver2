...
async function getContent(slug: string) {
  try {
    const contentPath = path.join(process.cwd(), 'content', 'es', 'impuestos-y-trabajo-autonomo', `${slug}.md`);
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const crumbs = [
      { name: "Home", path: "/es" },
      { name: "Impuestos y trabajo autónomo", path: "/es/impuestos-y-trabajo-autonomo" },
      { name: calculatorName }
  ];
