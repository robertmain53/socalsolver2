...
async function getContent(slug: string) {
  try {
    const contentPath = path.join(process.cwd(), 'content', 'it', 'famiglia-e-vita-quotidiana', `${slug}.md`);
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const crumbs = [
      { name: "Home", path: "/it" },
      { name: "Famiglia e Vita Quotidiana", path: "/it/famiglia-e-vita-quotidiana" },
      { name: calculatorName }
  ];
