...
export async function generateMetadata() {
  ...
  return generateSEOMetadata({
    title: `${categoryInfo?.name || 'Fisco e Lavoro Autonomo'} - Calcolatori Professionali`,
    description: `${calculators.length} calcolatori gratuiti per ${categoryInfo?.name.toLowerCase() || 'fisco e lavoro autonomo'}: tasse, contributi, regime forfettario, partita IVA e molto altro.`,
    keywords: [
      'calcolatori fisco',
      'partita iva',
      'regime forfettario',
      ...
      <div className="space-y-6 sm:space-y-8">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6 sm:p-8 md:p-12 rounded-xl sm:rounded-2xl text-center shadow-xl">
          ...
          <p className="text-base sm:text-lg md:text-xl opacity-90 max-w-3xl mx-auto">
            {calculators.length} calcolatori professionali gratuiti per{' '}
        ...
        <div className="flex justify-center">
          <div className="inline-flex items-center px-4 py-2 bg-blue-100 text-blue-800 rounded-full font-semibold text-sm">
            ...
            {calculators.length} Strumenti Disponibili
          </div>
        </div>
        ...
        {calculators.length === 0 && (
          <div className="text-center py-12 bg-gray-50 rounded-2xl">
            <p className="text-gray-600 text-lg">
              Nessun calcolatore disponibile in questa categoria al momento.
            </p>
          </div>
        )}
        ...
        <div className="bg-white p-6 sm:p-8 rounded-xl sm:rounded-2xl shadow-lg">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-4">
            Calcolatori per {categoryName}
          </h2>
          <div className="prose prose-sm sm:prose max-w-none text-gray-600">
            <p>
              La nostra collezione di calcolatori per <strong>{categoryName.toLowerCase()}</strong>{' '}
              ti aiuta a gestire in modo professionale e preciso tutti gli aspetti fiscali e
              contributivi del tuo lavoro autonomo.
            </p>
            <p>
              Tutti i nostri strumenti sono <strong>gratuiti</strong>, sempre{' '}
              <strong>aggiornati</strong> con le ultime normative e non richiedono registrazione.
            </p>
          </div>
        </div>
      </div>
