...
async function getContent(slug: string) {
  try {
    const contentPath = path.join(process.cwd(), 'content', 'fr', 'epargne-et-investissements', `${slug}.md`);
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const crumbs = [
      { name: "Home", path: "/fr" },
      { name: "Épargne et investissements", path: "/fr/epargne-et-investissements" },
      { name: calculatorName }
  ];
