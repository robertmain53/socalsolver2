...
async function getContent(slug: string) {
  try {
    const contentPath = path.join(process.cwd(), 'content', 'fr', 'famille-et-vie-quotidienne', `${slug}.md`);
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const crumbs = [
      { name: "Home", path: "/fr" },
      { name: "Famille et vie quotidienne", path: "/fr/famille-et-vie-quotidienne" },
      { name: calculatorName }
  ];
